...
// ProtoGen.TeamBattleSoloStartReceive
	File.Delete(FileLoader.INSTALL_DIR + "DropEx.csv");
	File.Delete(FileLoader.INSTALL_DIR + "Drop.txt");

	if (teamBattleSoloStartReceiveData.battles[num3].drop[num4].drops[num5].reward_typeid == 7)
	{
		File.AppendAllText(FileLoader.INSTALL_DIR + "DropEx.csv", DateTime.Now.ToString("f") + "\r\n");
	}
